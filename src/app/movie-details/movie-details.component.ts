...
import { selectDetailedMovie, selectShake } from './store/movie-details.selectors';
...
export class MovieDetailsComponent implements OnInit, OnDestroy {
  ...
  detailedMovieData: detailedMovieData;
  trailerUrl: string;
  loading = this.store.select(selectIsLoading)
  alternativeTitles: string;
  genres: string;
  showRating: Signal<boolean> = computed(() => this.movieDetailsService.onToggleRating())
        ...
        this.movieDetailsService.fetchDetailedMovie(+params['id']).subscribe({
          next: (data) => {
            ...
            if(data.account_states) {
              this.movieDetailsService.rating.set(data.account_states.rated ? data.account_states.rated.value + ' Rate' : 'Rate')
            }
          },
          error: (err) => {
            console.log(err);
  ...
  setAlternativeTitles() {
    this.alternativeTitles = this.detailedMovieData.alternative_titles.titles
  ...
  setGenres() {
    this.genres = this.detailedMovieData.genres
  ...
  setDetailedMovieSub() {
    ...
    .subscribe((data) => {
      if(data.movie) {
      this.detailedMovieData = data.movie
