...
import {
  ...
  setGenres,
  setMovies,
} from './store/movie-listings.actions';
...
export class MovieListingsService {
  ...
  searchTitle: WritableSignal<string | null> = signal(null);
  ...
  fetchGenres = () => {
    return this.http
      .get<genresData>(environment.GENRES_URL, {
        headers: {
  ...
  fetchMovies = (
    ...
    return this.http
      .get<moviesData>(environment.MOVIE_URL, {
        headers: {
          Authorization: 'Bearer ' + environment.API_READ_ACCESS_TOKEN,
        },
        params: params,
  ...
  searchMovies = (
    ...
    this.http
      .get<moviesData>(
        `${environment.SEARCH_MOVIE_URL}?${queryTitle}&include_adult=false&language=en-US&page=${page}`,
        {
          headers: {
            Authorization: 'Bearer ' + environment.API_READ_ACCESS_TOKEN,
          },
